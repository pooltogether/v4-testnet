...
.setAction(async (args, { ethers }) => {
    ...
    if (!balance.gte(ethers.utils.parseUnits(args.amount, decimals))) {
      debug(`Insufficient balance; minting...`)
      const txMint = await token.mint(wallet.address, ethers.utils.parseUnits(args.amount, decimals))
      await txMint.wait()
    }

    debug(`Approving...`)
    const txApprove = await token.connect(wallet).approve(prizePool.address, ethers.utils.parseUnits(args.amount, decimals))
    console.log(cyan(emoji.find('🕰️').emoji, ` Approving Deposit to PrizePool`))
    await txApprove.wait()

    debug(`Depositing...`)
    await prizePool.connect(wallet).depositTo(wallet.address, ethers.utils.parseUnits(args.amount, decimals))
