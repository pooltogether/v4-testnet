...
export default async function deployToArbitrumRinkeby(hardhat: HardhatRuntimeEnvironment) {
  if (process.env.DEPLOY === 'v1.1.0.arbitrumrinkeby') {
    dim(`Deploying: Receiver Chain Arbitrum Rinkeby`);
    dim(`Version: 1.1.0`);
  } else {
    return;
  }
